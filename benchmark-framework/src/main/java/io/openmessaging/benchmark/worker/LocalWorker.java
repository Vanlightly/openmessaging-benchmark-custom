...
import io.openmessaging.benchmark.metrics.PrometheusEndpoint;
...
public class LocalWorker implements Worker, ConsumerCallback {
    ...
    public static final long NonE2ELatencyMaxHistoValue = TimeUnit.MINUTES.toMicros(5);
    public static final long E2ELatencyMaxHistoValue = TimeUnit.HOURS.toMicros(12);

    private final Recorder publishLatencyRecorder = new Recorder(NonE2ELatencyMaxHistoValue, 5);
    private final OpStatsLogger publishLatencyStats;

    private final Recorder scheduleLatencyRecorder = new Recorder(NonE2ELatencyMaxHistoValue, 5);
    private final OpStatsLogger scheduleLatencyStats;

    private final Recorder publishDelayLatencyRecorder = new Recorder(NonE2ELatencyMaxHistoValue, 5);
    private final OpStatsLogger publishDelayLatencyStats;

    private final Recorder endToEndLatencyRecorder = new Recorder(E2ELatencyMaxHistoValue, 5);
    private final OpStatsLogger endToEndLatencyStats;

    private final PrometheusEndpoint metricsEndpoint;
    ...
    public LocalWorker(StatsLogger statsLogger) {
        ...
        this.metricsEndpoint = new PrometheusEndpoint();
        this.metricsEndpoint.run();
            ...
            try {
                while (!testCompleted) {
                    producers.forEach(producer -> {
                        ...
                        long scheduleMicroSafe = Math.min(scheduleMicros, NonE2ELatencyMaxHistoValue);

                        scheduleLatencyRecorder.recordValue(scheduleMicroSafe);
                        ...
                        f.thenRun(() -> {
                            ...
                            long latencyMicrosSafe = Math.min(latencyMicros, NonE2ELatencyMaxHistoValue-1);
                            publishLatencyRecorder.recordValue(latencyMicrosSafe);
                            ...
                            final long sendDelayMicrosSafe = Math.min(sendDelayMicros, NonE2ELatencyMaxHistoValue);
                            publishDelayLatencyRecorder.recordValue(sendDelayMicrosSafe);
    ...
    public PeriodStats getPeriodStats() {
        ...
        stats.timeoutErrors = timeoutErrors.sumThenReset();
        ...
        metricsEndpoint.updateStats(stats);
