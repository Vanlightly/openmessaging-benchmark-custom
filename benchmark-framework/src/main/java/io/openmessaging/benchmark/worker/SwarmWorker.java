...
public class SwarmWorker implements Worker {
    ...
    public PeriodStats getPeriodStats() {
        ...
        individualStats.forEach(is -> {
            ...
            stats.timeoutErrors += is.timeoutErrors;
            ...
            try {
                stats.publishLatency.add(Histogram.decodeFromByteBuffer(
                        ByteBuffer.wrap(is.publishLatencyBytes), NonE2ELatencyMaxHistoValue));
                
                stats.scheduleLatency.add(Histogram.decodeFromByteBuffer(
                    ByteBuffer.wrap(is.scheduleLatencyBytes), NonE2ELatencyMaxHistoValue));

                stats.publishDelayLatency.add(Histogram.decodeFromByteBuffer(
                        ByteBuffer.wrap(is.publishDelayLatencyBytes), NonE2ELatencyMaxHistoValue));

                stats.endToEndLatency.add(Histogram.decodeFromByteBuffer(
                        ByteBuffer.wrap(is.endToEndLatencyBytes), E2ELatencyMaxHistoValue));
            } catch (ArrayIndexOutOfBoundsException e) { //| DataFormatException e) {
